...
module.exports = function(express, settings, db){
    ...
    if (app.settings.env === 'test'){
        ...
        if (app.conf.redis_url) {
            console.info('[libby] sessions in redis provided by url');
            RedisStore = require('connect-redis')(session);
            sessionOpts.store = new RedisStore({
                url: app.conf.redis_url,
                ttl: app.conf.session_ttl
            });
        }
        else if (app.conf.redis && app.conf.redis.host !== undefined && app.conf.redis.port !== undefined){
